...
static int bug_int_perfctr(struct vperfctr *perfctr)
{
	current->thread.perfctr = NULL;
	BUG();
...
#endif
   .resume_userspace = bug_void_perfctr,
   .handle_signal = bug_void_perfctr,
   .handle_sysemu = bug_int_perfctr,
   .handle_tscemu = bug_int_perfctr,
   .handle_fpuemu = bug_int_perfctr,
};
