...
int gdns_ip4_packet(stralloc *out,const char *buf,unsigned int len)
{
  unsigned int pos;
  char header[12];
  uint16 numanswers;
  ...
  while (numanswers--) {
    ...
    pos = dns_packet_copy(buf,len,pos,header,10); if (!pos) return -1;
    uint16_unpack_big(header + 8,&datalen);
    /* if we have a proper A record, extract its contents */
    if (byte_equal(header,2,DNS_T_A)) {
      if (byte_equal(header + 2,2,DNS_C_IN))
        if (datalen == 4) {
	  if (!dns_packet_copy(buf,len,pos,header,4)) return -1; {
	    ...
	    len = ip4_fmt(str, header); 
	    str[len] = '\0';
	    if (!stralloc_cats(out," ")) return -1;
	    if (!stralloc_cats(out,str)) return -1;
	    if (!stralloc_cats(out," ")) return -1;
	    
	    uint32_unpack_big(header + 4, &ttl);

	    if (!stralloc_catlong0(out, (long)ttl, 7)) return -1;
	  }
	}
    } else if (byte_equal(header,2,DNS_T_NS)) {
      //      printf("found an NS record\n");
    }
    pos += datalen;
  }
