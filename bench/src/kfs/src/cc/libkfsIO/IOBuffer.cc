//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: IOBuffer.cc 368 2009-10-12 23:04:25Z sriramsrao $
//
// Created 2006/03/15
// Author: Sriram Rao
//
...
// ----- BEGIN ANNOTATION -----
// galtekar: Iterates through all buffer segments and labels them
// as data plane information.
void IOBuffer::MarkAllAsDataPlane() const
{
    BList::const_iterator it;
    int numBytes = 0;
    for (it = mBuf.begin(); it != mBuf.end(); ++it) {
        numBytes = it->BytesConsumable();
        VK_MARK_MEMORY(it->Consumer(), numBytes, VK_PLANE_DATA);
    }
}
// ----- END ANNOTATION -----
