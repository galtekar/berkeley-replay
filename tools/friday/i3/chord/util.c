...
/* subtract: a - b (mod 2^n) */
chordID
subtract(chordID a, chordID b)
{
    int borrow, i;
    chordID c;
	...
	if (a.x[i] - borrow < b.x[i]) {
	    c.x[i] = 256 + a.x[i] - borrow - b.x[i];
	    borrow = 1;
	} else {
	    c.x[i] = a.x[i] - borrow - b.x[i];
	    borrow = 0;
	}
    }
    return c;
...
void print_node(Node *node, char *prefix, char *suffix)
{
  ...
  printf(", %d, %d%s", node->addr, node->port, suffix);
}
