//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsRW_main.cc 353 2009-05-28 21:54:49Z sjakub $
//
// Created 2008/07/02
//
...
KfsClientPtr srcKfsClient, dstKfsClient;
static off_t doWrite(const string &kfspathname, int numMBytes, size_t writeSizeBytes);

void getHostPort(const string &hp, string &host, int &port)
{
    ...
    port = boost::lexical_cast<int> (rest);
}
...
main(int argc, char **argv)
{
    ...
    srcKfsClient = getKfsClientFactory()->GetClient(host, port);
    if (!srcKfsClient) {
    ...
    dstKfsClient = getKfsClientFactory()->GetClient(host, port);
    if (!dstKfsClient) {
