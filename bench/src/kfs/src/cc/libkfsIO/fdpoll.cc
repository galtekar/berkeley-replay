//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: fdpoll.cc 326 2009-04-25 00:45:50Z sriramsrao $
//
// Created 2009/03/16
...
public:
        ...
        int inWaitMilliSec)
    {
        ...
        dopoll.dp_fds	  = mPollVecPtr;
        dopoll.dp_nfds	  = theEventCount;
        dopoll.dp_timeout = inWaitMilliSec;
        const int theRet = ioctl(mDevpollFd, DP_POLL, &dopoll);
        if (theRet > 0) {
            mLastIdx = theRet;
        }
...
private:
        ...
        void* inUserDataPtr)
    {
        ...
        return (
            epoll_ctl(mEpollFd, inEpollOp, inFd, &theEpollEvent) ?
            errno : 0
        );
    }
