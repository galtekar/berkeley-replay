...
/* Called from entry.S, so can't put these in a header. */

asmlinkage int perfctr_syscall_emu()
{
	struct vperfctr *perfctr = current->thread.perfctr;

   return (perfctr && _vperfctr_handle_sysemu(perfctr));
}


asmlinkage void perfctr_resume_userspace()
{
   struct vperfctr *perfctr = current->thread.perfctr;

   if (perfctr) {
      _vperfctr_resume_userspace(perfctr);
   } else {
      /* Driver may not be loaded and hence stubs
       * may not be filled in. If you call them,
