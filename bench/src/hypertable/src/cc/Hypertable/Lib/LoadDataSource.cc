 ...
 *
 */
LoadDataSource::LoadDataSource(std::string fname, std::string row_key_column, std::string timestamp_column) : m_fname(fname), m_cur_line(0), m_line_buffer(0), m_hyperformat(false), m_leading_timestamps(false), m_row_index(0), m_timestamp_index(-1), m_timestamp(0) {
   string line, column_name;
   char *base, *ptr;
   int index = 0;

   VK_MARK_FILE_BY_NAME(fname.c_str(), VK_PLANE_DATA);
   ...
   //VK_MARK_DATA_REGION((ulong)base, strlen(base)+1);
