...
struct vperfctr {
    ...
    volatile struct vperfctr_state *kstate;
    int fd;
...
struct vperfctr *vperfctr_open_mapaddr(unsigned long page_start);
...
int vperfctr_close(struct vperfctr*);

/* Vkernel related calls. */
int vperfctr_emu_setup(const struct vperfctr *perfctr, 
      struct perfctr_emu_setup_struct *s);
int vperfctr_emu_enable(const struct vperfctr *perfctr);
int vperfctr_emu_disable(const struct vperfctr *perfctr);
int vperfctr_ctx_sigprocmask(const struct vperfctr *perfctr, struct ctx_maskinfo *m);
