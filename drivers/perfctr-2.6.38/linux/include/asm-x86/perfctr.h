/* $Id: perfctr.h,v 1.1.2.8 2008/11/30 16:33:51 mikpe Exp $
...
struct perfctr_cpu_state {
   ...
   /* Number of interrupts received by task at CPL 3 since
    * vpefctr was created (not including the create syscall). 
    * We can use this to derive precise branch retired counts on
    * Core2 and P4 machines (which is otherwise difficult because
    * the branch count includes interrupt counts as well--blame
    * Intel). With the interrupt count, we can simply subtract the 
    * number of interrupts received from the "branch retired
    * count" to get the true branch retired count. */
   unsigned long long intr_count;
...
#define SI_PERF_SYSEMU (0x80 | (1 << 31))
#define SI_PERF_TSCEMU (0x70 | (1 << 31))
#define SI_PERF_FPUEMU (0x60 | (1 << 31))
