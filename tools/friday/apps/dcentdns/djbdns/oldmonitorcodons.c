...
int main(int argc,char **argv)
{
  ...
  for (;numdone < numqueries;) {
	...
	if (!errorcode) {
	  ...
	  query[i].hitRate = 0;
	  query[i].avgHitDelay = 0;
	  query[i].medHitDelay = 0;
	  query[i].missRate = 0;
	  query[i].avgMissDelay = 0;
	  query[i].medMissDelay = 0;
	  ...
	  for (j=0; j<resMsg->header.ancount; j++) {
		...
		if (namelen == strlen("NumHits") && byte_equal(name, namelen, "NumHits")) {
		  numHits = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("AvgHitDelay") && byte_equal(name, namelen, "AvgHitDelay")) {
		  query[i].avgHitDelay = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("MedHitDelay") && byte_equal(name, namelen, "MedHitDelay")) {
		  query[i].medHitDelay = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("NumMisses") && byte_equal(name, namelen, "NumMisses")) {
		  numMisses = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("AvgMissDelay") && byte_equal(name, namelen, "AvgMissDelay")) {
		  query[i].avgMissDelay = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("MedMissDelay") && byte_equal(name, namelen, "MedMissDelay")) {
		  query[i].medMissDelay = (value > 0) ? value : 0;
		}
		else if (namelen == strlen("NumObjects") && byte_equal(name, namelen, "NumObjects")) {
	  ...
	  query[i].numQueries = numHits+numMisses;
	  query[i].hitRate = (query[i].numQueries == 0) ? 0 : 100*numHits/query[i].numQueries;
	  query[i].missRate = (query[i].numQueries == 0) ? 0 : 100*numMisses/query[i].numQueries;
	    
	  //printf("MonitorCoDoNS: Debug: name %s numQueries %f hitRate %f avgHitDelay %f medHitDelay %f missRate %f avgMissDelay %f medMissDelay %f numObjects %f\n", query[i].name, query[i].numQueries, query[i].hitRate, query[i].avgHitDelay, query[i].medHitDelay, query[i].missRate, query[i].avgMissDelay, query[i].medMissDelay, query[i].numObjects);
  ...
  for (i=0; i<numqueries; i++) {
    for (j=1; j<numqueries; j++) {
      if (sortedquery[j-1]->medHitDelay > sortedquery[j]->medHitDelay) {
  ...
  for (i=0; i<numqueries; i++) {
	...
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      ...
      if (sortedquery[i]->medHitDelay != 0) {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">%.2f ms</font></td>\n", sortedquery[i]->medHitDelay);
      }
      else {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      }
      if (sortedquery[i]->avgHitDelay != 0) {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">%.2f ms</font></td>\n", sortedquery[i]->avgHitDelay);
      }
      else {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      }
      if (sortedquery[i]->numQueries != 0) {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">%.2f %%</font></td>\n", sortedquery[i]->missRate);
      }
      else {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      }
      if (sortedquery[i]->medMissDelay != 0) {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">%.2f ms</font></td>\n", sortedquery[i]->medMissDelay);
      }
      else {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      }
      if (sortedquery[i]->avgMissDelay != 0) {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">%.2f ms</font></td>\n", sortedquery[i]->avgMissDelay);
      }
      else {
	fprintf(fpstts, "\t\t<td align=\"center\" width=\"120\" height=\"50\"><font face=\"Palatino Linotype\" color=\"#0000FF\">- -</font></td>\n");
      }
