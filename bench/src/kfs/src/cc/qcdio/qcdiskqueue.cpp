//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: qcdiskqueue.cpp 354 2009-05-28 22:01:17Z sjakub $
//
// Created 2008/11/11
      ...
      struct iovec* inIoVecPtr)
{
   ...
   while (theBufCnt > 0 && theError == kErrorNone) {
      ...
      if (theReadFlag) {
         ...
         VALGRIND_PRINTF("before write\n");
         const ssize_t theNWr = writev(theFd, inIoVecPtr, theIoVecCnt);
         VALGRIND_PRINTF("theNWr=%d theIoBytes=%u\n", theNWr, theIoBytes);
         if (theNWr > 0) {
            theIoByteCnt += theNWr;
         }
      ...
      bool        inCreateFlag)
{
   ...
   for (int i = theIdx; i < mFdCount; i += mFileCount) {
      ...
      // ----- BEGIN ANNOTATION -----
      // galtekar: The QCDiskQueue is used by (at least) the chunk
      // server to service chunk reads and writes in an asynchronous
      // fashion. Hence files in the queue should be part of the data-plane,
      // since they store KFS chunks. Note that we could use the
      // VK_MARK_FILE_BY_INO annotation here as well, but that requires
      // maintaining additional state in the vkernel, and we have to
      // explicitly remove the filename after it's unlinked. Too much
      // work. The _BY_FD macro will remove the marking when the file
      // object associated with the fd is deallocated.
      //
      // XXX: we need to verify that this is always a KFS chunk and not
      // something else (e.g., a config file) that should be considered
      // part of the control plane. Shouldn't be since config data
      // needn't be queued like this (it's generally small) and is 
      // typically read at the beginning of execution.
      VK_MARK_FILE_BY_FD(theFd, VK_PLANE_DATA);
      // ----- END ANNOTATION -----
