//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: RemoteSyncSM.cc 373 2009-10-12 23:24:00Z sriramsrao $
//
// Created 2006/09/27
// Author: Sriram Rao
//
...
RemoteSyncSM::HandleResponse(IOBuffer *iobuf, int msgLen)
{
    ...
    if (i != mDispatchedOps.end()) {
        ...
        if (op->op == CMD_READ) {
            ...
            // ----- BEGIN ANNOTATION -----
            // galtekar: at this point the chunkserver should've
            // gotten all bytes for the read operation made to the
            // other chunkserver (for replication)
            rop->dataBuf->MarkAllAsDataPlane();
            // ----- END ANNOTATION ------
        } else if (op->op == CMD_GET_CHUNK_METADATA) {
            GetChunkMetadataOp *gcm = static_cast<GetChunkMetadataOp *>(op);
            if (gcm->dataBuf == NULL)
