...
while (<FILE>){
	...
	foreach my $line (@lines){
		if ($line=~/^id/){
			...
			($node, $port)=split (/_/, $value);
			$address=$idValue."_".$value;
			$dir="exp.".$value;
			chomp ($dir);
		}
		if($neighborFlag==0 && $line!~/^neighbors/ ){
			if($line=~/^neighbor_file/){
				...
				$configText=$configText."polling_interval=5:0\n"."pv_interval=5:0\n";
				print $configText;
		...
		if($neighborFlag==1){
			...
			chomp($key);
			if ($line !~/#/) {
				$neighborText=$neighborText.$key."_".$map{$key}."\n";
			}
			print $neighborText;
		}
		if($line=~/^neighbors/){
	...
	print "Creating package for node: $node\n";
	if( -e $dir){
#		print ("rm -rf $dir\n");
		system("rm -rf $dir");
	}
...
#	print 	"mkdir $dir/servers\n";
#	system 	"mkdir $dir/servers\n";
#	print 	"cp $dnsdir/servers/@ $dir/servers/\n";
#	system 	"cp $dnsdir/servers/@ $dir/servers/\n";
#	system   "mkdir -p $dir/servers\n";
#	print 	"cp $dnsdir/servers/@ $dir/servers/\n";
#	system 	"cp $dnsdir/servers/@ $dir/servers/\n";
}
