//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsClient.cc 369 2009-10-12 23:06:41Z sriramsrao $
//
// Created 2006/04/18
// Author: Sriram Rao
//
...
{
    // ----- BEGIN ANNOTATION -----
    // galtekar: @buf may already be tainted due to a previously
    // executed annotation (e.g., the one in
    // KfsToolsCommon.cc::BackupFile2()), but we do one here in case we
    // missed an annotation point higher up.
    VK_MARK_MEMORY(buf, numBytes, VK_PLANE_DATA);
    // ----- END ANNOTATIONS -----
