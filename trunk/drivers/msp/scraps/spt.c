...
static pgd_t *
spgd_addr(struct shpt_cpu *cpu, u32 i, unsigned long vaddr)
{
...
static pgd_t *
gpgd_addr(struct shpt_cpu *cpu, unsigned long vaddr)
{
...
static void 
check_gpte(struct shpt_cpu *cpu, pte_t gpte)
{
...
static void 
check_gpgd(struct shpt_cpu *cpu, pgd_t gpgd)
{
#if 0
 ...
 * us. */
static unsigned int 
find_pgdir(struct shpt_cpu *cpu, unsigned long gpgdir)
{
   unsigned int i;
...
on_downgrade_ipi(void *arg)
{
   ...
   /* Notify user-mode of the downgrade. */
   event.kind = SHPT_EV_DOWNGRADE;
   event.pfn = pfn;
   event.vaddr = darg->vaddr;
   event.timestamp = darg->timestamp;
   shpt_queue_event(event, curr_sh);
...
static void
send_downgrade_ipi(struct shpt_cpu *cpu, unsigned long vaddr,
      int errcode, pte_t gpte, uint64_t timestamp)
{
   /* Must allocate args on heap rather tha stack since we don't wait for
    * IPI target to finish. We expect the IPI handler to free after use.
    *
    * XXX: performance bottleneck? static allocation faster? */
   struct downgrade_args *args = kmalloc(sizeof(*args), GFP_ATOMIC);
   ...
   ASSERT(cpu != curr_sh);
   ASSERT(cpu->id != curr_sh->id);

   /* We may deadlock if we enable IRQs here: imagine if we get an
    * interrupt that tries to acquire a task runqueue lock, even the the
    * runqueue lock is already held in the current context. */
#define ENABLE_IRQ_FOR_IPI 0
   ...
   smp_call_function_single(cpu->id, on_downgrade_ipi, (void*)args, 0);
...
static int
do_downgrade_work(struct shpt_cpu *rem_cpu, unsigned long gpgdir, 
...
static int
try_to_downgrade(struct shpt_cpu *rem_cpu, pte_t gpte, int errcode)
{
...
do_crew_downgrade(pte_t gpte, unsigned long vaddr, int errcode)
{
   ...
   for (i = 0; i < nr_cpu_ids; i++) {
      if (cpu_online(i) && i != smp_processor_id()) {
         int was_downgraded;
         struct shpt_cpu *rem_cpu = &shpt_state.cpus[i];
         was_downgraded = try_to_downgrade(rem_cpu, gpte, errcode);
         if (was_downgraded) {
            /* XXX: Should we send out IPIs all at once? Do we need to hold
             * shadow lock while doing so? */
            send_downgrade_ipi(rem_cpu, vaddr, errcode, gpte,
                  shpt_state.clock++);
         }
      }
   }
...
{
   return shpt_config.vkernel_len &&
      (shpt_config.vkernel_start <= vaddr &&
       vaddr < shpt_config.vkernel_start+shpt_config.vkernel_len);
}
...
static int
demand_page(struct shpt_cpu *cpu, unsigned long vaddr, int errcode)
{
   pgd_t *gpgd;
   pgd_t *spgd;
   pte_t *gpte_ptr, gpte;
    ...
    * shadow lock. */
   ASSERT(cpu == curr_sh);
   ASSERT(spin_is_locked(&shpt_state.lock));
   ...
   *gpte_ptr = gpte;
   ...
   if ((pte_flags(gpte) & _PAGE_USER) && !is_addr_in_vkernel(vaddr) &&
      ...
      do_crew_downgrade(gpte, vaddr, errcode);

      /* We've queued the downgrade events. Now queue an upgrade event. */
      event.kind = SHPT_EV_UPGRADE;
      event.vaddr = vaddr;
      event.pfn = pte_pfn(gpte);
      event.timestamp = shpt_state.clock++;
      /* XXX: ensure current gets it, not just any task in current cpu.
       * */
      shpt_queue_event(event, curr_sh);

      /* XXX: release lock ,if any downgraded, wake them */
   }
...
int
guest_page_fault(struct shpt_cpu *cpu, unsigned long vaddr, int errcode)
{
   int is_shadow_fault;
...
#if 0
   /* Initially, I thought that interrupts could be enabled here so we
    * can listen for invalidation IPIs. But doing so results in deadlock
    * with an IRQ handler. So it's disabled.*/
   while (!spin_trylock_irq(&shpt_state.lock));
#else
   shadow_lock();
#endif

   is_shadow_fault = demand_page(cpu, vaddr, errcode);

   /* Don't re-enable IRQs. */
   shadow_unlock();
...
static void
flush_user_mappings(struct shpt_cpu *cpu, int idx)
{
   unsigned int i;
   pgd_t *shpt_pgdir = cpu->pgdirs[idx].pgdir;
   if (shpt_pgdir) {
      /* Release every pgd entry up to the kernel's address. */
      for (i = 0; i < pgd_index(PAGE_OFFSET); i++) {
         release_pgd(shpt_pgdir + i);
      }
   }
}
...
void
guest_pagetable_flush_user(struct shpt_cpu *cpu)
{
   unsigned long flags;

   spin_lock_irqsave(&shpt_state.lock, flags);

   /* Drop the userspace part of the current page table. */
   flush_user_mappings(cpu, cpu->cpu_pgd);

   spin_unlock_irqrestore(&shpt_state.lock, flags);
}

static pteval_t
guest_pte_flags(struct shpt_cpu *cpu, unsigned long vaddr)
{
	pgd_t *gpgd;
	pte_t *gpte;
	/* First step: get the top-level Guest page table entry. */
...
static void
pin_page(struct shpt_cpu *cpu, unsigned long vaddr)
{
   pteval_t gflags;
   gflags = guest_pte_flags(cpu, vaddr);
   if (gflags & _PAGE_PRESENT) {
#if 1
      if (!demand_page(cpu, vaddr, (gflags & _PAGE_RW ? PF_WRITE : 0))) {
...
static void 
pin_kernel_pages(struct shpt_cpu *cpu)
{
   unsigned long vaddr;
 ...
 * blank_pgdir. */
static unsigned int 
new_pgdir(struct shpt_cpu *cpu, unsigned long gpgdir)
{
	unsigned int next;
...
int
guest_new_pagetable(struct shpt_cpu *cpu, unsigned long gpgdir)
{
	int idx;
   unsigned long flags;
...
static void 
release_all_pagetables(struct shpt_cpu *cpu)
{
   unsigned int i;
...
void 
guest_pagetable_clear_all(struct shpt_cpu *cpu, bool repin)
{
   unsigned long flags;
...
void 
guest_free_pagetable(struct shpt_cpu *cpu, unsigned long gpgdir)
{
	unsigned int i;
   unsigned long flags;
 ...
 */
static void
do_set_pte(struct shpt_cpu *cpu, int idx, unsigned long vaddr, pte_t gpte)
{
	/* Look up the matching shadow page directory entry. */
...
void
guest_set_pte(struct shpt_cpu *cpu, unsigned long gpgdir, unsigned long vaddr,
              pte_t gpte)
{
 ...
 */
void
guest_set_pmd(struct shpt_cpu *cpu, unsigned long gpgdir, u32 idx)
{
	int pgdir;
...
static void
shpt_global_write_cr3(unsigned long pgdir)
{
...
void
shpt_load_shpt_cr3(void)
{
   struct shpt_cpu * cpu = curr_sh;
   ...
   shpt_global_write_cr3(__pa(vpgdir));
}

void
shpt_switch_to(unsigned long gpgdir)
{
   /* Our CPUID interceptor should've disabled PSE. */
   ASSERT(!cpu_has_pse);
   ASSERT(gpgdir);

   if (SHPT_find_guest(gpgdir) != -1) {
      int idx;
