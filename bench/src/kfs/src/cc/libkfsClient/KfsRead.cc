//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsRead.cc 363 2009-07-21 18:02:50Z sriramsrao $
//
// Created 2006/10/02
// Author: Sriram Rao
//
...
KfsClientImpl::DoSmallReadFromServer(int fd, char *buf, size_t numBytes)
{
   ...
   // ----- BEGIN ANNOTATION -----
   // galtekar: This appears to be the lowest level in the stack
   // that we can annotate the data and yet incldue data plane
   // processing code (e.g., the following checksum
   // verification). We must be very conservative in order to
   // avoid polluting the results.
   VK_MARK_MEMORY(op.contentBuf, op.contentLength, VK_PLANE_DATA);
   // ----- END ANNOTATION -----
