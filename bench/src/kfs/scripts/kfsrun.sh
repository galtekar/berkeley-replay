#/bin/bash
#
# $Id: kfsrun.sh 239 2009-01-09 07:48:35Z sriramsrao $
#
...
startServer()
{
    ...
    #bdr-record bin/$server $config $SERVER_LOG_FILE > /dev/null 2>&1 &
    bdr-record bin/$server $config $SERVER_LOG_FILE &
    #bdr-record --debug-level=5 bin/$server $config &
	...
	RETVAL=$?
	echo
   # galtekar: added wait, otherwise ssh connection will terminate
   # and kill bdr
   wait
	return $RETVAL
    fi
