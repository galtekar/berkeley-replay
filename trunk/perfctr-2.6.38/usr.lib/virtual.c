...
static int vperfctr_open_pid(int pid, ulong page_start, struct vperfctr *perfctr, unsigned int mode)
{
   int fd, creat;
   ...
   perfctr->kstate = mmap((void*)page_start, PAGE_SIZE, PROT_READ | PROT_WRITE,
         (page_start ? MAP_FIXED : 0) | MAP_SHARED, perfctr->fd, 0);
...
_vperfctr_open_mode(ulong page_start, unsigned int mode)
{
   struct vperfctr *perfctr;

   perfctr = malloc(sizeof(*perfctr));
   if (perfctr) {
      if (vperfctr_open_pid(0, page_start, perfctr, mode) == 0)
         return perfctr;
      free(perfctr);
   }
   return NULL;
...
struct vperfctr *
vperfctr_open_mode(unsigned int mode)
{
   return _vperfctr_open_mode(0, mode);
}

struct vperfctr *vperfctr_open()
{
   return vperfctr_open_mode(VPERFCTR_OPEN_CREAT_EXCL);
}

struct vperfctr *vperfctr_open_mapaddr(ulong page_start)
{
   return _vperfctr_open_mode(page_start, VPERFCTR_OPEN_CREAT_EXCL);
}
...
int
vperfctr_emu_setup(const struct vperfctr *perfctr, 
      struct perfctr_emu_setup_struct *s)
{
   return ioctl(perfctr->fd, VPERFCTR_EMU_SETUP, s);
}

int
vperfctr_emu_enable(const struct vperfctr *perfctr)
{
   return ioctl(perfctr->fd, VPERFCTR_EMU_ENABLE);
}

int
vperfctr_emu_disable(const struct vperfctr *perfctr)
{
   return ioctl(perfctr->fd, VPERFCTR_EMU_DISABLE);
}

int
vperfctr_ctx_sigprocmask(const struct vperfctr *perfctr, struct ctx_maskinfo *m)
{
   return ioctl(perfctr->fd, VPERFCTR_CTX_SIGPROCMASK, m);
}
...
int 
vperfctr_close(struct vperfctr *perfctr)
{
   int err;

   err = munmap((void*)perfctr->kstate, PAGE_SIZE);
...
struct rvperfctr *rvperfctr_open(int pid)
{
   ...
   if (rvperfctr) {
      if (vperfctr_open_pid(pid, 0, &rvperfctr->vperfctr, VPERFCTR_OPEN_CREAT_EXCL) == 0) {
         rvperfctr->pid = pid;
         return rvperfctr;
      }
      free(rvperfctr);
