...
int get_entry(char *buf, char *end,
    	      char *ip_addr, int *port,	char *id,  char *status)
{
  char *ip_start, *port_start, *id_start, *status_start, *temp, *p1, *p2;
...
void get_address(char *web_url, I3ServerList *list)
{
  int sockid;
  int bufsize;
  char host[STR_LEN];
  ...
  if (!(hostaddr = gethostbyname(host))) {
    fprintf(stderr, "Getting server list: error resolving host.");
    return;
  }
  ...
  while ( (bufsize = read(sockid, buffer+offset, sizeof(buffer)-offset-1))) {
	...
	if (status) {
	    DEBUG(11, "%s %d %s\n", ip_addr, port, id);
	    update_i3server(list,
		    ntohl(inet_addr(ip_addr)), port, atoi3id(id));
	}
      }
      p = next + 1;
    }
