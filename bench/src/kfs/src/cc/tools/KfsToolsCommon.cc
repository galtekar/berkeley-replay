//---------------------------------------------------------- -*- Mode: C++ -*-
// $Id: KfsToolsCommon.cc 353 2009-05-28 21:54:49Z sjakub $
//
// Created 2009/04/18
//
...
BackupFile2(KfsClientPtr kfsClient, string srcfilename, string kfsfilename)
{
   ...
   // ----- BEGIN ANNOTATION -----
   // galtekar: srcFd is the fd of the file being copied into KFS.
   // The semantics of the file are clear to the user, but opaque
   // to the system. Hence it's part of the data plane.
   VK_MARK_FILE_BY_FD(srcFd, VK_PLANE_DATA);
   // ----- END ANNOTATIONS -----
...
RestoreFile2(KfsClientPtr kfsClient, string kfsfilename, string localfilename)
{
   ...
   while (1) {
      ...
      // ----- BEGIN ANNOTATION -----
      // galtekar: This is data received from a chunkserver, and so
      // it should've been marked as data plane by out annotations
      // in the KfsClient library. 
      VK_MARK_ASSERT_MEMORY(kfsBuf, nRead, VK_PLANE_DATA);
      // ----- END ANNOTATIONS -----
      n += nRead;
